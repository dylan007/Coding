...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> pence;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << pence << std::endl;
		std::cout.flush();

		__timer_start();

		BritishCoins __object;
		vector <int> __actual = __object.coins(pence);
