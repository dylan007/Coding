...
int	S;
		...
		__in >> __expected >> volume >> S;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << volume << ", " << S << std::endl;
		std::cout.flush();

		__timer_start();

		WakingUpEasy __object;
		int __actual = __object.countAlarms(volume, S);
