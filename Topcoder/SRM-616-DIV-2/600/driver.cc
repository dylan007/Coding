...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> values;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << values << std::endl;
		std::cout.flush();

		__timer_start();

		ColorfulCoinsEasy __object;
		string __actual = __object.isPossible(values);
