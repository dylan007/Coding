...
#include "RepeatNumberCompare.cc"
...
int	k2;
		...
		__in >> __expected >> x1 >> k1 >> x2 >> k2;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << x1 << ", " << k1 << ", " << x2 << ", " << k2 << std::endl;
		std::cout.flush();

		__timer_start();

		RepeatNumberCompare __object;
		string __actual = __object.compare(x1, k1, x2, k2);
