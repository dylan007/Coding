...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		vector <string>	__expected;
		string	card;

		__in >> __expected >> card;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << card << std::endl;
		std::cout.flush();

		__timer_start();

		MakePalindrome __object;
		vector <string> __actual = __object.constructMinimal(card);
