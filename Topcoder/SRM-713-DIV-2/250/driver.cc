...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> board;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << board << std::endl;
		std::cout.flush();

		__timer_start();

		SymmetryDetection __object;
		string __actual = __object.detect(board);
