...
int	k;
		...
		__in >> __expected >> image >> k;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << image << ", " << k << std::endl;
		std::cout.flush();

		__timer_start();

		ImageCompression __object;
		string __actual = __object.isPossible(image, k);
